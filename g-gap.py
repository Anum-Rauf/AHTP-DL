"""
    This is a g-gap protein feature extraction method to generate CSV format files 
...
"""
	...
	-g: It represents the counted rank (or tier) of the correlation along a protein sequence. It must be non-Negative integer and smaller than L-1 (required). 
	...
	!!!!!! 
	The input file must be in fasta format. The sample must be labeled in the following format: 
	
	>hormone|Q802V6|ABH2@1@
    MNTHESEVYTVA...PEMPAMFDGMKLAA...
	
	@1@ represents the label for this sequence is 1
...
def generateCsvFormatLine(in_file, out_file):
    ...
    for i in range(num_lines):
        if lines[i][0] == '>':
            sampleType = re.findall(r'_(\d)', lines[i])[0]
